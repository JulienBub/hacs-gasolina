...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    if service_info := async_last_service_info(hass, address):
        ...
        # Pre-populate bottle size from the cached advertisement so the entity
        # shows the correct value immediately – before the first live advertisement.
        if parsed is not None and parsed.bottle_size is not None:
            coordinator.bottle_size = parsed.bottle_size
